...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your backend-api account.<br>
        ...
        &mdash; backend-api Team
      `
      return sendMail({ toEmail: email, subject: 'backend-api - Password Reset', content })
    })
